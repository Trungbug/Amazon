...
function PlaceOrder(props) {
    // const { item, size, increment } = useContext();
    ...
    const fetchProductDetails = (productId) => {
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const product = fakeApiData.find(item => item.id == productId);
                if (product) {
                    resolve(product);
                } else {
    ...
    return (
        <div>
            <Grid container>
                <Grid item xs={6} >
                    <img src={productDetails.image} className="placeorder__image" />
                </Grid>
                <Grid item xs={3} >
                    <div className="placeorder__description">
                        ...
                        <div> <Rating name="read-only" value="3" readOnly style={{ fontSize: "20px" }} />
                            ...
                            {productDetails.review}+ answered questio
                        </div>
                        <hr></hr>
                        <div>
                            ...
                            <div className="textgap">EMI starts at :<strong>$ {productDetails.emi}</strong> . No Cost EMI available</div>
